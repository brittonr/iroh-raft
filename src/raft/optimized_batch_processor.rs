...
impl EnhancedRaftProposal {
    pub fn new(proposal: RaftProposal, priority: ProposalPriority) -> Result<Self> {
    ...
    fn estimate_proposal_size(data: &ProposalData) -> Result<usize> {
...
impl OptimizedRaftBatchProcessor {
    ...
    async fn process_proposal_chunk(&self, proposals: Vec<EnhancedRaftProposal>) -> Result<()> {
    ...
    async fn add_to_priority_batch(&self, proposal: EnhancedRaftProposal) -> Result<()> {
        // Check backpressure
        if self.config.enable_backpressure {
            let pending = self.pending_proposals.get();
            if pending >= self.config.max_pending_proposals as u64 {
                return Err(RaftError::ResourceExhausted {
    ...
    async fn flush_ready_batches(&self) -> Result<()> {
    ...
    async fn process_single_batch(&self, batch: OptimizedProposalBatch) -> Result<()> {
        if batch.is_empty() {
            return Ok(());
        }
    ...
    fn create_batched_proposal(&self, proposals: Vec<RaftProposal>) -> Result<RaftProposal> {
        ...
        let (batch_tx, batch_rx) = oneshot::channel::<Result<()>>();
        ...
        tokio::spawn(async move {
            match batch_rx.await {
                ...
                Ok(Err(e)) => {
                    ...
                    for (_id, tx) in response_channels {
                        let _ = tx.send(Err(RaftError::Internal {
