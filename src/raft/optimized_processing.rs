...
impl OptimizedRaftProcessor {
    ...
    pub async fn process_message_batch_ref(&self, batch: &MessageBatch<'_>) -> Result<Vec<Bytes>> {
    ...
    pub async fn queue_message(&self, message: Message) -> Result<()> {
    ...
    pub async fn process_next_batch(&self) -> Result<Option<Vec<Bytes>>> {
    ...
    async fn process_batch_owned(&self, messages: Vec<Message>, priority: BatchPriority) -> Result<Vec<Bytes>> {
