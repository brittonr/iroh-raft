...
impl RaftEventLoop {
    ...
    async fn handle_tick(&self, tick_count: &mut u64) -> Result<()> {
        *tick_count += 1;
...
mod tests {
    ...
    impl ProposalHandler for MockProposalHandler {
        async fn handle_proposal(&self, _proposal: RaftProposal) -> Result<()> {
            Ok(())
        }
    }
    ...
    impl ConfChangeHandler for MockConfChangeHandler {
        async fn handle_conf_change(&self, _conf_change: RaftConfChange) -> Result<()> {
            Ok(())
        }
    }
