...
type PendingProposals = Arc<RwLock<HashMap<Vec<u8>, oneshot::Sender<Result<()>>>>>;
...
impl RaftManager {
    ...
    pub fn new(
        ...
        let raft_node =
            RawNode::new(&cfg, storage.clone(), &logger).map_err(|e| RaftError::Raft {
    ...
    fn setup_batch_processing(
        ...
        shared_state: &Weak<crate::node_shared::SharedNodeState>,
        proposal_tx: ProposalChannel,
    ) -> Result<ProposalChannel> {
        if let Some(_shared) = shared_state.upgrade() {
            let config = config_global::get().map_err(|e| RaftError::Internal {
    ...
    pub async fn apply_initial_conf_state(&self, conf_state: raft::prelude::ConfState) -> Result<()> {
        info!(
            self.logger,
    ...
    async fn handle_proposal(&self, proposal: RaftProposal) -> Result<()> {
        #[cfg(feature = "observability")]
        ...
        let data = bincode::serialize(&proposal.data).map_err(|e| RaftError::Serialization {
        ...
        // Propose through Raft
        {
            ...
            if node.raft.state != StateRole::Leader {
                ...
                if let Some(response_tx) = pending.remove(&proposal.id) {
                    let _ = response_tx.send(Err(RaftError::NotLeader {
    ...
    async fn handle_conf_change(&self, conf_change: RaftConfChange) -> Result<()> {
        let mut node = self.raft_node.write().await;
        self.config_manager
    ...
    #[instrument(skip(self))]
    async fn on_ready(&self) -> Result<bool> {
        let mut node = self.raft_node.write().await;
    ...
    /// Send messages to peers
    async fn send_messages_to_peers(&self, ready: &mut Ready) -> Result<()> {
        if !ready.messages().is_empty() {
            info!(
                self.logger,
    ...
    async fn process_normal_entry(&self, entry: &Entry) -> Result<()> {
    ...
    async fn process_conf_change_entry(
        ...
        if let Err(e) = cc.merge_from_bytes(&entry.data) {
            ...
            if !entry.context.is_empty() {
                let mut pending = self.pending_proposals.write().await;
                if let Some(response_tx) = pending.remove(&entry.context) {
                    let _ = response_tx.send(Err(RaftError::Internal {
    ...
    async fn send_raft_message(&self, mut msg: raft::prelude::Message) -> Result<()> {
