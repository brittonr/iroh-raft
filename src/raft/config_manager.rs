...
impl RaftConfigManager {
    ...
    pub async fn handle_conf_change(
        ...
        let context_data =
            bincode::serialize(&context).map_err(|e| RaftError::Serialization {
        ...
        match raft_node.propose_conf_change(vec![], cc.clone()) {
            ...
            Err(e) => {
                ...
                if let Some(response_tx) = conf_change.response_tx {
                    let _ = response_tx.send(Err(RaftError::Raft {
                        operation: "propose configuration change".to_string(),
                        source: Box::new(e),
                    }));
                }
    ...
    async fn update_local_peers_after_conf_change(&self, cc: &ConfChange) -> Result<()> {
                        ...
                        Ok(context) => {
                            ...
                            if let Some(shared) = self.shared_state.upgrade() {
                                if context.p2p_node_id.is_some() || !context.p2p_addresses.is_empty() {
                                    ...
                                    // P2P connection info is stored, connections established on-demand
                                    if cc.node_id != self.node_id {
                                        if let Some(ref _p2p_node_id) = context.p2p_node_id {
                                            info!(
                                                self.logger,
    ...
    async fn save_conf_state(&self, conf_state: &raft::prelude::ConfState) -> Result<()> {
        let write_txn = self
            .database
            .begin_write()
...

        {
            ...
            let conf_data = bincode::serialize(&serializable_conf_state)
                .map_err(|e| RaftError::serialization("serialize conf state", e))?;
    ...
    fn create_node_addr_with_relay(
        ...
        relay_url: &str,
    ) -> Result<iroh::NodeAddr> {
        use std::str::FromStr;

        let node_id = iroh::NodeId::from_str(p2p_node_id).map_err(|e| RaftError::Internal {
