...
impl RedbRaftStorage {
    ...
    pub fn append(&self, entries: &[raft::prelude::Entry]) -> Result<()> {
        #[cfg(feature = "failpoints")]
    ...
    /// Save hard state
    pub fn save_hard_state(&self, hard_state: &raft::prelude::HardState) -> Result<()> {
        #[cfg(feature = "observability")]
        let metrics = safe_metrics().ok();
        #[cfg(feature = "observability")]
        let _timer = metrics.as_ref().map(|m| Timer::with_attributes(
    ...
    pub fn save_conf_state(&self, conf_state: &raft::prelude::ConfState) -> Result<()> {
        tracing::info!(
    ...
    pub fn initialize_single_node(&self, node_id: u64) -> Result<()> {
    ...
    pub fn load_conf_state(&self) -> Result<raft::prelude::ConfState> {
        let read_txn = self.database.begin_read()?;
    ...
    pub fn create_snapshot_data(&self) -> Result<SnapshotData> {
        #[cfg(feature = "failpoints")]
    ...
    pub fn restore_from_snapshot(&self, data: &[u8]) -> Result<()> {
        let snapshot_data: SnapshotData =
            bincode::deserialize(data).map_err(|e| RaftError::Serialization {
    ...
    pub fn apply_snapshot(&self, snapshot: &raft::prelude::Snapshot) -> Result<()> {
    ...
    pub fn compact_log_before(&self, index: u64) -> Result<()> {
        if index == 0 {
            return Ok(());
        }
    ...
    pub fn save_tenant_quota(
        ...
        let write_txn = self.database.begin_write()?;
        {
            ...
            let serialized =
                bincode::serialize(quota).map_err(|e| RaftError::Serialization {
    ...
    pub fn get_tenant_quota(
        &self,
        tenant_id: &str,
    ) -> Result<Option<crate::resource_quotas::TenantQuota>> {
        let read_txn = self.database.begin_read()?;
    ...
    pub fn get_all_quotas(&self) -> Result<Vec<crate::resource_quotas::TenantQuota>> {
        let read_txn = self.database.begin_read()?;
    ...
    pub fn delete_tenant_quota(&self, tenant_id: &str) -> Result<()> {
        let write_txn = self.database.begin_write()?;
        {
            let mut table = write_txn.open_table(TENANT_QUOTA_TABLE)?;
    ...
    pub fn save_node_topology(
        ...
        let write_txn = self.database.begin_write()?;
        {
            ...
            let serialized =
                bincode::serialize(topology).map_err(|e| RaftError::Serialization {
    ...
    pub fn get_node_topology(
        &self,
        node_id: u64,
    ) -> Result<Option<crate::types::NodeTopology>> {
        let read_txn = self.database.begin_read()?;
...
pub fn init_database_tables(database: &Arc<Database>) -> Result<()> {
...
pub fn init_raft(node_id: u64, database: Arc<Database>) -> Result<RawNode<RedbRaftStorage>> {
    ...
    RawNode::new(&raft_config, storage, &logger).map_err(|e| RaftError::Raft {
...
pub trait Storage: Send + Sync + std::fmt::Debug {
    ...
    /// Get all tenant quotas from storage
    async fn get_all_quotas(&self) -> Result<Vec<crate::resource_quotas::TenantQuota>>;

    /// Delete tenant quota from storage
    async fn delete_tenant_quota(&self, tenant_id: &str) -> Result<()>;
}
